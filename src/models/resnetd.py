...
class Stem(nn.Sequential):
    def __init__(self, c1, ch, c2):
        super().__init__(
            ...
            nn.BatchNorm2d(c2),
            nn.ReLU(True),
            nn.MaxPool2d(3, 2, 1)   ### stride was 2 if 1 the utput image is 128*128
        )
...
class ResNetD(nn.Module):
    def __init__(self, model_name: str, num_input_ch: int) -> None:
        super().__init__()
        ...
        self.stem1 = Stem(self.num_input_ch, 64, self.inplanes)
        self.layer1 = self._make_layer(block, 64, depths[0], s=1)
        self.layer2 = self._make_layer(block, 128, depths[1], s=2)   ### stride was 2
        self.layer3 = self._make_layer(block, 256, depths[2], s=1, d=2)    ### stride was 2
        self.layer4 = self._make_layer(block, 512, depths[3], s=1, d=4)   ### stride was 2
