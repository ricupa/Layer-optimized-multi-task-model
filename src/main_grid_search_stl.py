...
def main():
    ...
    # Calculate 10 values for lambda in logarithmic scale between 0.0001 and 0.001
    lambda_values = np.round(np.logspace(np.log10(0.0001), np.log10(0.001), 10), 6)
    # lambda_values = np.linspace(0.0001, 0.001, 10) ### for linear scale

    
    config['init_Experiment_name'] = config['Experiment_name']
    
    for _lambda in lambda_values:
        
        config['bb_optimizer_params']['lambda'] = _lambda
        print('lambda = ', config['bb_optimizer_params']['lambda'])
        
        config['Experiment_name'] = config['init_Experiment_name'] + '_'+ str(_lambda)
        ...
        fname = config['checkpoint_folder'] + config['dataset_name']+ '/'+ config['backbone']+ '/stl/' + config['Experiment_name'] + '_trial_0'
        print(fname)
        ...
        runs = wandb.init(project= tempname, name=config['Experiment_name']+ '_trial_0', entity='ricupa', config=config, dir = fname, reinit=True)
